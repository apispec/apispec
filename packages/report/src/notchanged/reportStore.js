import {
  extendObservable, observable, action, autorun, configure,
} from 'mobx';
...
class ReportStore {
  constructor(data = {}, config = {}) {
    ...
    extendObservable(
      this,
      {
        ...
        showPending:
          config.showPending !== undefined ? config.showPending : true,
        showSkipped:
  ...
  _mapTest = (test) => ((this.showPassed && test.pass)
  ...
  _mapSuite = (suite) => {
    ...
    return beforeHooks.length
      ...
      || suites.length
      ? ({
        ...suite, suites, beforeHooks, afterHooks, tests,
      })
      : null;
  };
  ...
  _getFilteredTests = (suite) => transduce(suite, this._mapSuite, this._reduceItem, []);
  ...
  _isValidShowHookOption = (option) => this._isValidOption('showHooks', this.showHooksOptions, option);
...
export const log = (store) => autorun(() => console.log('UP', store.results));
